...
exports.AVEDEV = function() {
  var range = utils.parseNumberArray(_.flatten(arguments));
...
exports.AVERAGE = function() {
  var range = utils.numbers(_.flatten(arguments));
  var n = range.length;
  var sum = 0;
  var count = 0;
  for (var i = 0; i < n; i++) {
...
exports.AVERAGEIF = function(range, criteria, average_range) {
  ...
  average_range = utils.parseNumberArray(_.flatten(average_range));
...
exports.CORREL = function(array1, array2) {
  array1 = utils.parseNumberArray(_.flatten(array1));
  if (array1 instanceof Error) {
    return array1;
  }
  array2 = utils.parseNumberArray(_.flatten(array2));
  if (array2 instanceof Error) {
    return array2;
  }
...
exports.COUNT = function() {
  var numbers = utils.numbers(_.flatten(arguments));
...
exports.COUNTIFS = function() {
  var args = utils.argsToArray(arguments);
  var results = new Array(_.flatten(args[0]).length);
  for (var i = 0; i < results.length; i++) {
    results[i] = true;
  }
  for (i = 0; i < args.length; i += 2) {
    var range = _.flatten(args[i]);
    ...
    for (var j = 0; j < range.length; j++) {
      if (typeof range[j] !== 'string') {
        results[j] = results[j] && evalExpr(range[j] + criteria);
      } else {
        results[j] = results[j] && evalExpr('"' + range[j] + '"' + criteria);
      }
    }
...
exports.FORECAST = function(x, data_y, data_x) {
  ...
  data_y = utils.parseNumberArray(_.flatten(data_y));
  if (data_y instanceof Error) {
    return data_y;
  }
  data_x = utils.parseNumberArray(_.flatten(data_x));
  if (data_x instanceof Error) {
    return data_x;
  }
...
exports.FREQUENCY = function(data, bins) {
  data = utils.parseNumberArray(_.flatten(data));
  if (data instanceof Error) {
    return data;
  }
  bins = utils.parseNumberArray(_.flatten(bins));
...
exports.GEOMEAN = function() {
  var args = utils.parseNumberArray(_.flatten(arguments));
...
exports.LARGE = function(range, k) {
  range = utils.parseNumberArray(_.flatten(range));
  if (range instanceof Error) {
    return range;
  }
  k = utils.parseNumber(k);
...
exports.MAXA = function() {
  var range = utils.arrayValuesToNumbers(_.flatten(arguments));
  return (range.length === 0) ? 0 : Math.max.apply(Math, range);
};
...
exports.PERCENTILE.EXC = function(array, k) {
  array = utils.parseNumberArray(_.flatten(array));
...
exports.PROB = function(range, probability, lower, upper) {
  ...
  probability = utils.parseNumberArray(_.flatten(probability));
  if (probability instanceof Error) {
    return probability;
  }
