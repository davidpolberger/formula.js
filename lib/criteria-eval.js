...
exports.matchWithWildcards = function (pattern, text) {
  ...
  for (var patternPos = 0; patternPos < pattern.length; ++patternPos) {
    ...
    if (patternCh === '*' && pattern[patternPos - 1] !== '~') {
      while (pattern[patternPos + 1] === '*')
        ++patternPos;
      if (patternPos === pattern.length - 1)
        return true;
      var nextPatternCh = pattern[patternPos + 1];
      while (text[textPos++] !== nextPatternCh) {
        if (textPos >= text.length)
          return false;
      }
      --textPos;
    } else if (patternCh === '?' && pattern[patternPos - 1] !== '~') {
      ++textPos;
    } else {
...
//exports.matchWithWildcardsUsingRegExp = function (left, right) {
