...
exports.MINVERSE = function(x) {
  ...
  var s = utils.matrix.dim(x), m = s[0], n = s[1];
  var A = x, Ai, Aj;
  var I = utils.matrix.identity(m), Ii, Ij;
  var i,j,k;
  for(j=0;j<n;++j) {
    var i0 = -1;
    var v0 = -1;
...
exports.MMULT = function(x, y) {
  ...
  var d = utils.matrix.dim;
  switch(d(x).length*1000+d(y).length) {
  case 2002:
    if(y.length < 10) {
      return utils.matrix.dotMMsmall(x,y);
    }
    return utils.matrix.dotMMbig(x,y);
  case 2001: return utils.matrix.dotMV(x,y);
  case 1002: return utils.matrix.dotVM(x,y);
  case 1001: return utils.matrix.dotVV(x,y);
  case 1000: return utils.matrix.mulVS(x,y);
  case 1: return utils.matrix.mulSV(x,y);
  case 0: return x*y;
...
exports.MUNIT = function(dimension) {
  ...
  return utils.matrix.identity(dimension);
};
