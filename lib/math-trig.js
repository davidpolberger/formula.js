...
/**
 * Excel does not allow this function to be entered into a cell with either range or sum_range
...
exports.SUMIF = function (range, criteria, sum_range) {
  ...
  if (sum_range === undefined) sum_range = range;

  range = utils.arrayify(range);
  sum_range = utils.arrayify(sum_range);
  if (sum_range.length !== range.length)
    throw new Error('range and sum_range must have same number of elements.');
